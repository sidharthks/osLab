...
int main() {
    ...
    if (pid > 0) { // Parent process
        ...
        // Temporary storage for palindromes
        char palindromeBuffer[BUFFER_SIZE];
        int palindromeCount = 0;

        // Read all strings from the pipe
        while (read(pipefd[0], buffer, BUFFER_SIZE) > 0) { // Read from the pipe
            if (isPalindrome(buffer)) {
                strcpy(palindromeBuffer + palindromeCount, buffer);
                palindromeCount += strlen(buffer) + 1;
            }
        }

        // Write palindromes back to the pipe
        if (palindromeCount > 0) {
            write(STDOUT_FILENO, palindromeBuffer, palindromeCount);
        } else {
